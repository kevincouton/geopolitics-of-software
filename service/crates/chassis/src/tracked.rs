...
pub async fn list_for_user(
    ...
    sqlx::query_as!(
        ...
        SELECT p.id, p.github_owner, p.github_name, p.gitee_owner, p.gitee_name, p.language,
               ...
               COALESCE(ds.asia_readiness_score, NULL) AS score, t.created_at AS tracked_at
...
pub async fn untrack(pool: &DbPool, user_id: &str, project_id: Uuid) -> Result<(), ApiError> {
    let result = sqlx::query!(
        "DELETE FROM tracked_projects WHERE user_id = $1 AND project_id = $2",
        user_id,
        project_id
    )
    .execute(pool)
