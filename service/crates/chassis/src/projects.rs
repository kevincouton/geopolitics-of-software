...
pub async fn upsert(pool: &DbPool, repo: &GithubRepo) -> Result<Project, ApiError> {
    sqlx::query_as!(
        ...
        RETURNING id, github_owner, github_name, gitee_owner, gitee_name, language, topics, description,
                  stars, forks, open_issues, has_chinese_readme, has_gitee_mirror,
                  NULL::INTEGER AS score, created_at, updated_at
        "#,
        repo.owner.login,
        repo.name,
        repo.description,
...
pub async fn list(pool: &DbPool, limit: i64) -> Result<Vec<Project>, ApiError> {
    sqlx::query_as!(
        Project,
        r#"
        SELECT id, github_owner, github_name, gitee_owner, gitee_name, language, topics, description,
               stars, forks, open_issues, has_chinese_readme, has_gitee_mirror,
               NULL::INTEGER AS score, created_at, updated_at
        FROM projects
        ORDER BY stars DESC
